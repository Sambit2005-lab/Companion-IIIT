...
public class hostside_Ecell_societies_calendar extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hostside_ecell_societies_calendar);
        ...
        if (hostEmail != null) {
            if (hostEmail.equals("ecell@iiit-bh.ac.in")) {
                hostEventsRef = databaseReference.child("5").child("events");
            } else {
    ...
    private void saveEventToFirebase() {
        ...
        if (eventId != null) {
            hostside_Ecell_societies_calendar.Event event = new hostside_Ecell_societies_calendar.Event(date, description);
            hostEventsRef.child(eventId).setValue(event)
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            Toast.makeText(hostside_Ecell_societies_calendar.this, "Event saved successfully", Toast.LENGTH_SHORT).show();
                            finish();
                        } else {
                            Toast.makeText(hostside_Ecell_societies_calendar.this, "Failed to save event", Toast.LENGTH_SHORT).show();
                        }
                    });
        }
    }
