...
public class hostside_codexnova_societies_calendar extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_hostside_codexnova_societies_calendar);
        ...
        if (hostEmail != null) {
            if (hostEmail.equals("code.x.novas@gmail.com")) {
                hostEventsRef = databaseReference.child("1").child("events");
            } else {
            ...
            hostEventsRef.child(eventId).setValue(event)
                    .addOnCompleteListener(task -> {
                        if (task.isSuccessful()) {
                            Toast.makeText(hostside_codexnova_societies_calendar.this, "Event saved successfully", Toast.LENGTH_SHORT).show();
                            finish();
                        } else {
                            Toast.makeText(hostside_codexnova_societies_calendar.this, "Failed to save event", Toast.LENGTH_SHORT).show();
                        }
                    });
        }
    }
